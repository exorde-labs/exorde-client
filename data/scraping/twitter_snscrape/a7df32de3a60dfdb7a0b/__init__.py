...
async def generate_twitter_url(keyword, live_mode=True):
    base_url = "https://twitter.com/search?q={}&src=typed_query".format(str(keyword))
    if live_mode:
...
async def get_sns_tweets(search_keyword, select_top_tweets, nb_tweets_wanted) -> AsyncGenerator[Item, None]:
    """
    ...
    for _post in snscrape.modules.twitter.TwitterSearchScraper('{} since:{}'.format(search_keyword, today), mode=mode).get_items():
        post = _post.__dict__
        ...
        if tr_post["creationDateTime"] is not None:
            newness = is_within_timeframe_seconds(tr_post["creationDateTime"],120)
            if not newness:
...
async def query(url: str) -> AsyncGenerator[Item, None]:
    ...
    print("\nSearch_keyword = ",search_keyword, " select_top_tweets = ", select_top_tweets, nb_tweets_wanted)
    print("Scraping....")
    async for result in get_sns_tweets(search_keyword, select_top_tweets, nb_tweets_wanted):
        yield result
