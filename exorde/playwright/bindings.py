...
from aiosow.bindings import (
    wrap, each, on, setup, option
)

from . import * 

option('no_headless', action='store_true', default=False, help='Wether playwright should run in headless mode')
option(
    'user_agent',
...
pages = wrap(
    lambda pages: {
        'pages' : { index: pages[index] for index in range(0, len(pages)) } 
    }
)
